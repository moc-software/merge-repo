...
class CenterProduct(models.Model):
    ...
    hsn_number = models.CharField(max_length=20, default = 'Abc')
    price = models.IntegerField(default=500)
    center = models.ForeignKey(Center,on_delete= models.CASCADE)
...
class Service(models.Model):
    ...
    services = models.ManyToManyField(CenterServices , blank=True , null=True)
    ...
    additional_services_cost = models.IntegerField(default=0 , null=True , blank=True)
    ...
    def save(self):
        ...
        # Calculating cost of center services
        total_service_price = 0
        for service in list(self.services.all()):
            cost = service.price * (1 + (service.tax/100))
            total_service_price += cost
        total_product_price = 0
        pro_sales = ProductSale.objects.filter(service = self)
        for sale in pro_sales:
            cost = sale.product.price + (1+(sale.product.tax/100))
            total_product_price += cost
        final = (total_service_price + total_service_price + self.additional_services_cost) * (1 - (self.discount/100))
